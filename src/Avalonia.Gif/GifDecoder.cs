...
// Licensed under the Apache License, Version 2.0 (https://www.apche.org/licenses/LICENSE-2.0)
...
public sealed class GifDecoder : IDisposable
{
    ...
    private void RenderFrameAt(int idx, WriteableBitmap writeableBitmap)
    {
        ...
        DecompressFrameToIndexBuffer(curFrame, _backupFrameIndexBuf, tmpB);

        if (_hasFrameBackups & curFrame.ShouldBackup)
        {
            Buffer.BlockCopy(_backupFrameIndexBuf, 0, _backupFrameIndexBuf, 0, curFrame.Dimensions.TotalPixels);
            _backupFrame = idx;
        }

        DrawFrame(curFrame, _backupFrameIndexBuf);
