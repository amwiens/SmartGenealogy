...
public class ServiceManager<T>
{
        ...
        where TService : T
    {
        ...
        lock (instances)
        {
            ...
            {
                throw new ArgumentException($"Service of type {typeof(TService)} is already registered for {typeof(T)}");
            }
        ...
        where TService : T
    {
        ...
        if (providers.TryGetValue(typeof(TService), out var provider))
        {
            ...
            if (result is null)
            {
                throw new ArgumentException($"Service provider for {typeof(TService)} returned null");
            }
