...
using SmartGenealogy.Core.Exceptions;
    ...
    IDisposable
{
        ...
        bool isPersistent)
    {
        ...
        switch (option)
        {
            ...
            default:
                logger.LogError("Unknown notification option {Options}", option);
                break;
        }
    }
    ...
    public void ShowPersistent(
        ...
        Microsoft.Extensions.Logging.LogLevel logLevel = Microsoft.Extensions.Logging.LogLevel.Warning)
    {
        // Log exception
        ...
        NotificationType appearance = NotificationType.Error)
    {
        ...
        {
            Show(new Notification(title, message ?? ex.Message, appearance));
            return TaskResult<T>.FromException(ex);
        }
    }
        ...
        NotificationType appearance = NotificationType.Error)
    {
        ...
        catch (Exception ex)
        {
            ...
            Show(new Notification(title, message ?? ex.Message, appearance));
            return new TaskResult<bool>(false, ex);
        }
    }
    ...
    public async Task<INotificationManager?> GetNativeNotificationManagerAsync()
    {
        ...
        try
        {
            if (Compat.IsWindows)
            {
                var context = WindowsApplicationContext.FromCurrentProcess("Smart Genealogy");
        ...
        {
            logger.LogError(ex, "Failed to initialize native notification manager");
        }
