...
public partial class ConsoleViewModel : ObservableObject, IDisposable, IAsyncDisposable
{
    ...
    public async Task StopUpdatesAsync()
    {
        ...
        {
            // We can still continue since this just means we lose some remaining output
            Logger.Warn("Buffer completion timed out: " + ex.Message);
        }
    ...
    private async Task ConsoleUpdateLoop()
    {
        ...
        try
        {
            while (!ct.IsCancellationRequested)
            {
                ...
                catch (InvalidOperationException ex)
                {
                    ...
                    throw new OperationCanceledException("Update buffer completed", ex);
                }
        ...
        {
            Logger.Debug($"Console update loop canceled: {ex.Message}");
        }
        catch (Exception ex)
        {
            // Log other errors and continue here to not crash the UI thread
            Logger.Error(ex, $"Unexpected error in console update loop: {ex.GetType().Name} {ex.Message}");
        }
    }
    ...
    /// </summary>
    /// <remarks>This call will hold an upgradeable read lock</remarks>
    ...
    public void Dispose()
    {
        ...
        if (updateTask is not null)
        {
            ...
            {
                Logger.Warn(ex, "During shutdown - Console update task dispose failed");
            }
        }
